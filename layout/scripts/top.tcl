...
# rca_64bit. The P&R flow is based on the EDI 13.1 official workshop and use the
...
set init_verilog "designs/rca_64bit.mapped.v"
...
floorPlan -r 3.8 0.6 20 20 24 24

# Load IO assignment file: rca_64bit.ioc
# It specifies the relative location of module pins and on which metal layer you
# are preferred to place your module pins
loadIoFile "scripts/rca_64bit.ioc" -noAdjustDieSize
# Save the checkpoint for floorplan
saveDesign db/rca_64bit_floorplan.enc
...
# starting from the left offset 20.0 (roughly middle of the rca_64bit module)
...
saveDesign db/rca_64bit_powerplan.enc
...
saveDesign db/rca_64bit_place.enc
...
specifyClockTree -create {AutoCTSRootPin cin End}	
# Set CTS engine: ck
...
# Calculates the delay for one path based on maximum operating condition while calculating the delay for another path based on minimum operating condition for setup or hold checks
setAnalysisMode -analysisType onChipVariation

# Remove clock reconvergence pessimism for both setup and hold modes
...
saveDesign db/rca_64bit_cts.enc
...
saveDesign db/rca_64bit_postroute.enc
...
verifyGeometry -report reports/rca_64bit.geometry.rpt
verifyConnectivity -type all -report reports/rca_64bit.connectivity.rpt
...
report_area -out_file reports/rca_64bit.routed.area.rpt

# Power report
report_power -outfile reports/rca_64bit.routed.power.rpt

# ------------------------------------------------------------------------------
# Step 7b. Exports the design
# ------------------------------------------------------------------------------
# SPEF file: parastic data of wires
rcOut -spef results/rca_64bit.spef -rc_corner rc_worst

# SDF file: delay files for gates and wires
write_sdf results/rca_64bit.sdf

# *.v: verilog netlist file
saveNetlist results/rca_64bit.routed.v

# *.gds: GDS file
streamOut results/rca_64bit.gds -mapFile streamOut.map \
